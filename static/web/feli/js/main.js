...
var Gacetilla_Module = {
	...
	methods: function () {
		...
		function countList() {
			console.log('[array productos] - Lista de PRODUCTOS', this.getAll());
			return Gacetilla_Module.attrs.publicaciones.length;
		}

		function display() {
			$('.publicaciones_seleccionadas').text(this.countList());
			console.log('[array publicaciones] - Lista de PRODUCTOS', this.getAll());
		}
		...
		function insert(id) {
				console.log('ACA'); 
				var data = Gacetilla_Module.attrs.publicaciones.length;
			if (this.checkAvailableListPublicaciones(id)) {
				console.log('no estas');
				console.log(data);
//					$("div.floating-wpp-button").trigger('mouseover');
...
jQuery(function ($) {
    ...
    $(document).on('click', '.fa-plus', function() {
			...
			var total = parseInt($("span#cant_"+id).html()) +1;
//alert(total);
			$("span#cant_"+id).html(total);
		});   
	...
	$(document).on('click', '#enviar', function(e) {
		...
		$('#cart').modal('hide');
		var data_msj = ''; 
		var nombre = "\n -------------   \n nombre:"+localStorage.getItem("_nombre")+' \n  email: '+localStorage.getItem("_email");
		$('div.floating-wpp-message').html("Hola "+localStorage.getItem("_nombre")+" espero tu pedido !!");
		
		$( "div.badges > div " ).each(function( index ) {
		index++;
...
//		$("div.floating-wpp-button").trigger('mouseover');
		$('div.floating-wpp-input-message textarea').val(data_msj+nombre);
		$("div.floating-wpp-btn-send").trigger('click');
    ...
    $(document).on('click', 'a[data-target="#agregar"]', function() {
			
			Gacetilla_Module.methods().insert($(this).data('id'));
			var pit = $(this).data('id');
			var divs = $(" div.badges > div[id='"+$(this).data('id')+"']").length;
			if(divs > 0){
				
				var total = parseInt( divs) + parseInt( $("span#cant_"+$(this).data('id')).html());
				$("span#cant_"+$(this).data('id')).html(total);
			
			}else{
//				var total;
			$("div.badges").append('<div id="'+$(this).data('id')+'" > <span id="'+$(this).data('id')+'" class="badge" style="font-size: 18px;">Producto '+$(this).data('id')+' X <span id="cant_'+$(this).data('id')+'">1</span>  </span> <span class="badge"><i data-id_prod="'+$(this).data('id')+'" class="fas fa-plus"></i></span><span  class="badge"><i data-id_prod="'+$(this).data('id')+'" class="fas fa-minus"></i></span></div>');
			}
			console.log('totla'+$(this).data('id'));
			check_empty_cart();
	...
	$(document).on('click', 'a[data-target="#cart"]', function() { 
			
				if(localStorage.getItem("_nombre") != null) {
					$("input[id='nombre']").val();
				}else{
					$("input[id='nombre']").attr("placeholder", "tu nombre");
				}
	    ...
	    $(document).on('click', '#continuar', function (e) {
			e.preventDefault();
				var error = false;
			if($("input[id='nombre']").val()!= "" && $("input[id='email']").val()!= ""){
				if(validateEmail($("input[id='email']").val())){
					
					$("span#error_data").html('<span class="form-alert valid">;)</span>');
						localStorage.setItem("_email", $("input[id='email']").val());
						localStorage.setItem("_nombre", $("input[id='nombre']").val());
						check_user();
						check_empty_cart();
//					 $('#cart').modal('hide');
					
				}else{
					$("span#error_data").html('<span class="form-alert invalid">Ingresa un email válido</span>');
				}
			}else{
					$("span#error_data").html('<span class="form-alert invalid">Ingresa tus datos</span>');
...
function check_empty_cart(){
	...
	var divs = $(" div.badges > div").length;
	if(divs > 0 && (localStorage.getItem("_nombre") != null && localStorage.getItem("_email") != null)){
		console.log('divs');
		$("#enviar").removeAttr('disabled');
		$("#enviar2").removeAttr('disabled');
		$("#enviar").removeClass('invisible');
		$("#enviar2").removeClass('invisible');
	
		$("span#error_data").html('<span class="form-alert valid">ok ! ;)</span>');
	}else{
...
function check_user(){
	...
	if (localStorage.getItem("_nombre") != null && localStorage.getItem("_email") != null) {
		
		console.log('existe');
		console.log(localStorage.getItem("_nombre"));
	
		$("input[id='nombre']").val(localStorage.getItem("_nombre"));
		$("input[id='email']").val(localStorage.getItem("_email"));
		$("body").attr('user', true);
//			("#enviar").show();
		$("#continuar").addClass('invisible');
	
	}else{

//		$("div#register").css('display', 'block');
//		$("div#register").css('padding-right', '0px');
		$("body").attr('user', false);
		$("input[id='nombre']").attr("placeholder", "tu nombre");
		$("input[id='email']").attr("placeholder", "tu email");
		$("#enviar").attr('disabled','disabled');
		$("#enviar2").attr('disabled','disabled');
		
		$("#enviar").addClass('invisible');
		$("#enviar2").addClass('invisible');
		
		
		console.log('no wexiste');
	}
}	